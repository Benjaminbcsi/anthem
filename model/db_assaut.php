...
class AssautManager{
	...
	function db_getAssaut($id_javelin,$id_type,$limit=0,$offset=0,$order="id asc") {
		$stmt="SELECT * FROM `assaut` WHERE id_javelin=? and id_type=?";
		$query=$this->mysqli->prepare($stmt) or trace("Probleme avec ".$query->error);
		if ($query) {
			$query->bind_param("ii",$id_javelin,$id_type);
			$query->execute();
	...
	function db_getAssautbyid($id_arme,$limit=0,$offset=0,$order="id asc") {
		$stmt="SELECT * FROM `assaut` WHERE id=?";
		$query=$this->mysqli->prepare($stmt) or trace("Probleme avec ".$query->error);
		if ($query) {
