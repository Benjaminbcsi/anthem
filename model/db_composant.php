...
class ComposantManager{
	...
	function db_getComposant($id,$limit=0,$offset=0,$order="id asc") {
		$stmt="SELECT * FROM `composant` WHERE id_javelin=? or id_javelin = null";
	...
	function db_getComposantbyid($id_composant,$limit=0,$offset=0,$order="id asc") {
		$stmt="SELECT * FROM `composant` WHERE id=?";
		$query=$this->mysqli->prepare($stmt) or trace("Probleme avec ".$query->error);
		if ($query) {
			$query->bind_param("i",$id_composant);
			$query->execute();
			return $query->get_result();
		} else {
			return 0;
